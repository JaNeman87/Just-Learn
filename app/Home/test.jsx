...
const Test = () => {
    ...
    // --- REFACTOR: Track historical completion ---
    ...
    // --- REFACTOR: Save Logic ---
    const saveProgress = async (nextIndex, correct, incorrect, forceCompletion = false) => {
        if (parsedTest.id === "bookmark-practice-single") return;
        try {
            const progressJson = await AsyncStorage.getItem(PROGRESS_KEY);
            const allProgress = progressJson ? JSON.parse(progressJson) : {};

            // Determine if completed based on flags, history, or index
            const previousEntry = allProgress[parsedTest.id];
            const alreadyDone =
                wasAlreadyCompleted || previousEntry?.completed || previousEntry?.index >= parsedTest.questions.length;
            const isCompleted = forceCompletion || alreadyDone;

            allProgress[parsedTest.id] = {
                index: nextIndex,
                correct: correct,
                incorrect: incorrect,
                completed: isCompleted, // ALWAYS persist the completed flag
                timestamp: Date.now(),
            };
    ...
    // --- REFACTOR: Clear Logic (Start Over) ---
    const clearProgress = async () => {
        try {
            const progressJson = await AsyncStorage.getItem(PROGRESS_KEY);
            if (progressJson) {
                ...
                // If previously completed, DO NOT DELETE. Just reset index/score.
                // This keeps the test 'unlocked' for future tests.
                if (wasAlreadyCompleted || allProgress[parsedTest.id]?.completed) {
                    allProgress[parsedTest.id] = {
                        index: 0,
                        correct: 0,
                        incorrect: 0,
                        completed: true, // Maintain completed status
                    };
                } else {
                    // If never completed, we can remove it entirely
                    delete allProgress[parsedTest.id];
                }
    ...
    useEffect(() => {
        const checkProgress = async () => {
            ...
            try {
                ...
                // 1. Enforce "Only one test in progress"
                // Remove any OTHER tests that are NOT completed.
                let didCleanup = false;
                Object.keys(allProgress).forEach(key => {
                    if (key !== parsedTest.id) {
                        const entry = allProgress[key];
                        // If logic: if it's not marked completed, wipe it.
                        // (Assuming legacy data without 'completed' flag is considered incomplete if we want strict enforcement)
                        if (!entry.completed) {
                ...
                if (savedState) {
                    const isDone = savedState.completed || savedState.index >= parsedTest.questions.length;
                    setWasAlreadyCompleted(isDone);

                    // Only prompt resume if we have progress AND we are not "done" (unless user wants to review?
                    // Usually we start over if done, but here we just check if mid-way)
                    // If isDone is true, savedState.index might be max.
                    // If user is retrying, clearProgress (Start Over) would have set index to 0.

                    if (savedState.index > 0 && savedState.index < parsedTest.questions.length) {
                        setSavedProgress(savedState);
                        setShowResumeModal(true);
                    }
                }
    ...
    const handleCloseStatsModal = () => {
        ...
        // --- REFACTOR: Force completion flag on finish ---
        saveProgress(parsedTest.questions.length, correctAnswers, incorrectAnswers, true);
